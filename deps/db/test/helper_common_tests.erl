...
extract_length_coded_binary_testing() ->
	...
	?assert(helper_common:extract_length_coded_binary(<<7:8, 0:56, 77:80>>) =:= {<<0:56>>, <<77:80>>}).

extract_length_coded_string_testing() ->
	?debug_Msg(">>> extract_length_coded_string_testing()"),
	?assert(helper_common:extract_length_coded_string(<<11:8, "test string", 77:80>>) =:= {"test string", <<77:80>>}).
