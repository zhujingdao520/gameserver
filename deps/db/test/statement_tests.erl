...
statementInsertOne(_X, C) -> {timeout, 30, fun() ->
	...
	FieldValues = [
		...
		#mysql_time{neg=true,hour=200,minute=45,second=33},#mysql_time{year=1980,month=12,day=25,hour=23,minute=55},
				2023,#mysql_decimal{int="979797", fraction="0003203"},23.875547E-32,
		10.45E-7,25625,255,7,"char[20]",
...
%	?debug_Fmt("~p~n", [R3]),
	...
	[V1] = R3,
	case FieldName of
		_ when is_float(FieldValue) =:= true -> ?assert(abs(FieldValue - V1) < 1.0E-9);
		"binary_col" -> 
...
parameterCheck(Connection, [{Field_name, Type, Value, Exception} | Rest]) -> 
	?debug_Fmt(" >>> Statement 7. statementParameterCheck(), value=~p", [Value]),
