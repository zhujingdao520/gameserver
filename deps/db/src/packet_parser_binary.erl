...
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_DATE}, _Flags) ->
  ...
  case V0 of
    ...
    _ -> <<Year:16/little-integer, Month:8/integer, Day:8/integer>> = V0
  end,  
...
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_TIME}, _Flags) ->
  ...
  case Sec_part of 
    <<>> -> Second_part = 0;  
    _ -> <<Second_part:32/little-integer>> = Sec_part
  end,
...
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_VARCHAR}, _Flags) ->
  helper_common:extract_length_coded_string(B);
...
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_NEWDECIMAL}, _Flags) ->
  {String, B1} = helper_common:extract_length_coded_string(B),
  case string:tokens(String, ".") of
    [Int, Fract] -> true;
    [Int] -> Fract = "";
    _ ->  Int = "", Fract = ""
  end,  
  {#mysql_decimal{int=Int, fraction=Fract}, B1};
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_BLOB}, {Is_binary, _, _, _}) ->
  if
    Is_binary -> helper_common:extract_length_coded_binary(B);
    true      -> helper_common:extract_length_coded_string(B)
  end;
