...
-export([
  ...
  extract_length_coded_binary/1,
  extract_length_coded_string/1,
...
%% @spec extract_length_coded_binary(Binary::binary()) -> {Value::binary(), Rest_of_binary::binary()} 
%%
%% @doc Extracts lenght coded binary from binary stream and returns a tuple that contains
%% an extracted binary and rest of original binary.
%%
extract_length_coded_binary(B) ->
...
%% @spec extract_length_coded_string(Binary::binary()) -> {Value::string(), Rest_of_binary::binary()}
%%
%% @doc Extracts lenght coded string from binary stream and returns a tuple that contains
%% an extracted string and rest of original binary.
%%
extract_length_coded_string(B) ->
  case extract_length_coded_binary(B) of
    {null, B1} -> {null, B1};
    {Bytes, B1} -> {binary_to_list(Bytes), B1}
  end.
...
%%
binary_to_datetime(B) ->
  <<FieldSize:8/integer, V0:FieldSize/binary, B1/binary>> = B,
  case FieldSize of
    0 -> {#mysql_time{}, B1};
    _ -> <<Year:16/little-integer, Month:8/integer, Day:8/integer, V1/binary>> = V0,
      case V1 of
        <<>> -> Hour = 0, Minute = 0, Second = 0, Second_Part = 0;
        _ -> <<Hour:8/integer, Minute:8/integer, Second:8/integer, SecPart/binary>> = V1,
          case SecPart of
            <<>> -> Second_Part = 0;  
      _     -> <<Second_Part:32/little-integer>> = SecPart 
          end
      end,
...
time_to_lc_binary(#mysql_time{neg = Neg, day = Day, hour = Hour, minute = Minute, second = Second, second_part = Second_Part}) ->
  ...
  Negative = case Neg of true -> 1; false -> 0 end,
  DHMS = if
