...
is_open(Connection_pid) ->
  case is_process_alive(Connection_pid) of
    true -> gen_server:call(Connection_pid, is_open);
    false -> false
  end.

%% @spec execute_query(Connection_pid::pid(), Query::string()) -> Result
%% Result = {Metadata::#metadata{}, ResultSet} | #mysql_error{}
%% ResultSet = list(integer() | float() | string() | binary() | #mysql_time{} | #mysql_decimal{})
%%
...
transaction(Connection_pid, F) ->
  {_,[#ok_packet{}]} = connection:execute_query(Connection_pid, "START TRANSACTION"),
  R = try 
    F(Connection_pid)
  catch
...
%  io:format(">>> transaction ~p~n", [R]),
  case R of
    #mysql_error{} -> 
      {_,[#ok_packet{}]} = connection:execute_query(Connection_pid, "ROLLBACK"), 
      R#mysql_error{type = transaction};
    _ -> {_,[#ok_packet{}]} = connection:execute_query(Connection_pid, "COMMIT"), R
  end.
...
gen_server_call(Conn_pid, Request) ->
  try gen_server:call(Conn_pid, Request, ?GEN_SERVER_TIMEOUT)
    ...
    exit:Reason -> #mysql_error{type = connection, 
              source = "Exit is catched in connection:gen_server_call/2.",
              message = Reason}
  end.
