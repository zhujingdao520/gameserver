...
format(T, F, A, Mod, Line) ->
    ...
    case Line > 0 of
        false -> erlang:iolist_to_binary(io_lib:format(lists:concat(["## ", T, " ~s ", F, "~n"]), [Date] ++ A));
        true -> erlang:iolist_to_binary(io_lib:format(lists:concat(["## ", T, " ~s[~w:~w] ", F, "~n"]), [Date, Mod, Line] ++ A))
    end.

%% @doc 日志记录函数
-spec log(iodata()) -> ok.
log(Msg) ->
    db:execute("insert into log_error (msg) values(~s)", [Msg]),
    ok.
